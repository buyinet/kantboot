<template>
  <el-card>
    ...
    <el-table
      ...
      <el-table-column prop="content" label="角色描述"/>

      <el-table-column fixed="right" label="操作" width="200">
        <template #default="scope">
          <el-button link size="small" v-if="scope.row.defaultUse" disabled>注册时角色</el-button>
          <el-popconfirm
              :title="'确定使用 ['+scope.row.zhName+'] 为注册时角色吗?'"
              confirmButtonText="确定"
              cancelButtonText="取消"
              v-if="!scope.row.defaultUse"
              @confirm="setDefaultUse(scope.row.id)">
            <template #reference>
              <el-button link size="small">设置成注册时角色</el-button>
            </template>
          </el-popconfirm>
...
export default {
  ...
  methods: {
    setDefaultUse(id){
      this.$request.post({
        url: this.$api.role.setDefaultUse,
        data:{
          "id":id
        },
        stateSuccess: (res) => {
    ...
    findCommonPage() {
      ...
      this.$request.post({
        url: this.$api.role.findCommonPage,
        data: this.paramData,
        stateSuccess: (res) => {
