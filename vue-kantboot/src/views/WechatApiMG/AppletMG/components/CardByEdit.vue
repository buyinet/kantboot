    ...
    >
      ...
      <el-form
        ...
        <el-form-item label="APP ID">
          <el-input v-model="paramData.appId"/>
        </el-form-item>

        <el-form-item label="APP Secret">
          <el-input v-model="paramData.appSecret"/>
        </el-form-item>
        ...
        <el-form-item label="对应商户号">
          <el-select
              v-model="paramData.wechatPayParamId"
              style="width: 100%">
            <el-option
                v-for="item in pays"
                :key="item.id"
                :label="item.title+' ['+item.mchId+']'"
                :value="item.id"/>
          </el-select>
        </el-form-item>
  ...
  data() {
    return {
      paramData: {
        ...
        "appSecret": "",
        "content": "",
        "wechatPayParamId":null
      },
      pays: {}
    }
  },
  mounted() {
    this.findCommonListByPayWechatPay();
  },
  methods: {
    findCommonListByPayWechatPay() {
      this.$request.post({
        url: this.$api.tpWechatAppletPayParam.findCommonList,
        data: {"entity": {}},
        stateSuccess: (res) => {
          this.pays = res.data;
        },
    ...
    toSave() {
      ...
      this.$request.post({
        url: this.$api.tpWechatAppletParam.save,
        data: this.paramData,
        stateSuccess: (res) => {
