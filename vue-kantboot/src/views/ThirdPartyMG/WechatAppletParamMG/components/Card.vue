<template>
  <el-card>
    ...
    <el-table
      ...
      <el-table-column prop="wechatPayParam.title" label="对应商户号">
        <template #default="scope">
          <span v-if="wechatPayParam!=null">
			  {{scope.row.wechatPayParam.title}}
			    <br>
			    {{"["+scope.row.wechatPayParam.mchId+"]" }}
      ...
      <el-table-column fixed="right" label="操作" width="240">
        <template #default="scope">
          <el-button link size="small" v-if="scope.row.defaultUse" disabled>默认小程序ID</el-button>
          <el-popconfirm
              :title="'确定使用 ['+scope.row.title+'] 为默认小程序ID吗?'"
              confirmButtonText="确定"
              cancelButtonText="取消"
              v-if="!scope.row.defaultUse"
              @confirm="setDefaultUse(scope.row.id)">
            <template #reference>
              <el-button link size="small">设置成默认小程序ID</el-button>
            </template>
          </el-popconfirm>
...
export default {
  ...
  methods: {
    setDefaultUse(id){
      this.$request.post({
        url: this.$api.tpWechatAppletParam.setDefaultUse,
        data:{
          "id":id
        },
        stateSuccess: (res) => {
    ...
    removeById(id){
      this.$request.post({
        url: this.$api.tpWechatAppletParam.remove,
        data: {"id":id},
        stateSuccess: (res) => {
    ...
    findCommonPage() {
      ...
      this.$request.post({
        url: this.$api.tpWechatAppletParam.findCommonPage,
        data: this.paramData,
        stateSuccess: (res) => {
