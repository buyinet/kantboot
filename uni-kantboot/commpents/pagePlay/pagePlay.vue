...
<!-- 			<video style="width: 750rpx;height: 400rpx;
		...
		<view style="padding: 30rpx 30rpx 0 30rpx;font-size: 30rpx;">
			<u-subsection bgColor="#5EB2F3"
			 v-if="bodyData.fileUrlsOfVideo.length!=0"
			 :activeColor="'#5EB2F3'" :inactiveColor="'#FFFFFF'" :list="list1"
		...
		translateY(-50%);;">
			...
			style="
				...
				">该视频只有1集</view>
			<view
			v-if="bodyData.fileUrlsOfVideo.length!=1&&bodyData.fileUrlsOfVideo.length!=0"
			style="
				font-size: 28rpx;
				color:rgba(0,0,0,.4);
				text-align: center;
		...
		">
				<u-icon
				hoverClass="share-icon"
				@click="toPage('/pages/indexToShare/indexToShare?ranfaWorkId='+bodyData.id)"
				 name="share-fill" size="50">分享</u-icon>
		</view>
	</view>
</template>
